...
def load_real_samples(n_dimensions=3):
    ...
    # introduce the 3rd dimension to a single sample
...
def generate_real_samples(dataset, n_samples):
    ...
    X = dataset[ix]
    # label real samples with (-1's) - suited for the loss function
    y = -ones((n_samples,1))
    return X, y
...
class ClipConstraint(Constraint):
    # constraint the weight values within (-clip_value, clip_value)
...
# critic
# extract features and classify the input
def define_critic(in_shape=(3,3,1)):
    init = RandomNormal(stddev=0.02)
    const = ClipConstraint(0.01)
    model = Sequential()
    # process an entire input at once with (3,3) kernel resulting in 8 filters only due to low pattern diversity
    ...
    # BATCH NORMALISATION -- stabilises changes to the output values and loss calculations by making it sensitive
    # to the changes in the parameters' values [>>???<<]
...
# generator; latent_dim defines the number of random input variables used to generate the output
def define_generator(latent_dim):
    init = RandomNormal(stddev=0.03)
    model = Sequential()
    # n_nodes -> channels * height * width
    n_nodes = 8 * 3 * 3
    # latent_dim defines the number of input variables used to generate the output;
    # each pixel depends on all latent_dim
    model.add(Dense(n_nodes, kernel_initializer=init, input_dim=latent_dim))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Reshape((3, 3, 8)))
    # build structured feature maps from latent representations using transposed convolution
    model.add(Conv2DTranspose(8, (3,3), strides=(1,1), padding='same', kernel_initializer=init))
    model.add(BatchNormalization())
    model.add(LeakyReLU(alpha=0.2))
    # represent the output as a single feature map combining all features maps into one
    model.add(Conv2D(1, (3,3), activation='tanh', padding='same', kernel_initializer=init))
    return model


# GAN definition
# generator training phase: train the generator, keep the critic fixed
def define_gan(generator, critic):
    # freeze critic's weights but so that only the generator is updates in combined model
...
# generate n_samples latent random vectors of dimension latent_dim
...
def generate_fake_samples(generator, latent_dim, n_samples):
    ...
    # set all the fake samples to 1's that aligns with real samples being set to (-1's)
    y = ones((n_samples, 1))
    return X, y
...
def train(g_model, c_model, gan_model, dataset, latent_dim, n_epochs=500, n_batch=4, n_critic=5):
    ...
    for i in range(n_training_steps):
        ...
        # label generated samples as real so the loss measures how far the critic's score is from real
        # the larger the divergence from real, the more intense the generator's training
        y_gan = -ones((n_batch, 1))
        g_loss = gan_model.train_on_batch(X_gan, y_gan)
        g_history.append(g_loss)

        # keep track of the training process and summarize the performance
        print('&gt;%d, critic_real_loss=%.3f, critic_fake_loss=%.3f generator_loss=%.3f' % (i+1, c_real_history[-1], c_fake_history[-1], g_loss))
        if (i+1) % 10 == 0:
...
def summarize_performance(step, g_model, latent_dim, n_samples=100):
    ...
    # rescale the pixel values back to [0, 1]
    X = (X + 1) / 2.0
    for i in range(n_samples):
        # create a grid of 100 samples
